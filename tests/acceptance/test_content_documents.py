...
def test_xhtml_reference_to_undeclared_resource(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/content-xhtml-img-srcset-undeclared-error"), transport="subprocess")
    assert result.returncode == 0
...
def test_xhtml_escaped_hyperlinks(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/content-xhtml-link-to-local-file-escaped-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_xhtml_hyperlink_to_missing_resource(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/content-xhtml-link-to-missing-doc-error"), transport="subprocess")
    assert result.returncode == 1
...
def test_xhtml_style_element(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/style-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_xhtml_style_attribute(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/style-attr-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_xhtml_svg_image_reference(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/content-xhtml-img-fragment-svg-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_xhtml_mathml_formula(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/content-xhtml-mathml-altimg-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_xhtml_epub_type_attribute(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/epubtype-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_xhtml_custom_namespace_attributes(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/attrs-custom-ns-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_xhtml_its_attributes(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/attrs-its-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_css_minimal_stylesheet(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/content-css-minimal-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_css_direction_property(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/content-css-property-direction-error"), transport="subprocess")
    assert result.returncode == 1
...
def test_css_valid_selectors(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/content-css-selectors-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_css_syntax_errors(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/content-css-syntax-error"), transport="subprocess")
    assert result.returncode == 1
...
def test_svg_in_spine(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/content-svg-in-spine-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_svg_any_extension(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/content-svg-file-extension-unusual-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_svg_links_allowed(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/link-valid.svg"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_svg_duplicate_id(run_pyepubcheck, fixtures) -> None:
    """Report duplicate id attribute values."""
    result = run_pyepubcheck(_path(fixtures, "/epub3/06-content-document/files/id-duplicate-error.svg"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
