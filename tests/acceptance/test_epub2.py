...
def test_missing_mimetype_reports_pkg_006(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "epub/ocf-mimetype-missing-error"), transport="subprocess")
...
def test_wrong_default_namespace_reports_rsc_005(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "opf-document/xml-namespace-wrongdefault-error.opf"), transport="subprocess")
    assert result.returncode == 1
...
def test_item_href_with_spaces_reports_pkg_010(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "opf-document/item-href-contains-spaces-warning.opf"), transport="subprocess")
    assert result.returncode == 0
...
def test_resource_not_in_manifest_usage(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--mode", "exp", _epub2_fixture(fixtures, "epub/opf-manifest-resource-undeclared-usage"), transport="subprocess")
    assert result.returncode == 0
...
def test_hyperlinked_document_not_in_spine(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "epub/ops-xhtml-hyperlink-to-missing-fragment-error"), transport="subprocess")
    assert result.returncode == 1
...
def test_ops_xhtml_custom_namespace(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-xhtml/custom-ns-attr-error.xhtml"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_ops_svg_namespaced_extensions(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-svg/namespace-extension-valid.svg"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_ops_xhtml_minimal(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-xhtml/minimal.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_manifest_os_files_ignored(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "epub/opf-manifest-os-files-ignore-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_remote_object_undeclared(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "epub/opf-remote-object-undeclared-error"), transport="subprocess")
    assert result.returncode == 1
...
def test_metadata_creator_role_clr_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "epub/opf-metadata-creator-role-clr-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_package_id_spaces_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "epub/opf-package-id-spaces-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_ocf_filename_with_space_warning(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "epub/ocf-filename-with-space-warning"), transport="subprocess")
    assert result.returncode == 0
    assert result.has_warning("PKG-010")
...
def test_ocf_metainf_container_alternative_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "epub/ocf-metainf-container-alternative-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_ocf_mimetype_with_spaces(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "epub/ocf-mimetype-with-spaces-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("PKG-007")
...
def test_ops_xhtml_unusual_extension(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "epub/ops-xhtml-unusual-extension-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_ops_xhtml_doctype_unresolved_entity(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-xhtml/doctype-unresolved-entity-error.xhtml"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_ops_xhtml_entities_unknown(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-xhtml/entities-unknown-error.xhtml"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_ops_xhtml_svg_foreignObject_with_html(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-xhtml/svg-foreignObject-with-html-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_ops_xhtml_svg_foreignObject_switch(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-xhtml/svg-foreignObject-switch-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_ops_xhtml_class_empty(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-xhtml/class-empty-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_ops_xhtml_lang_attr(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-xhtml/lang-attr-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_ops_xhtml_entities_character_references(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-xhtml/entities-character-references-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_ops_xhtml_edit_attributes(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-xhtml/edit-attributes-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_ops_xhtml_edit_block_content(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-xhtml/edit-block-content-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_ops_xhtml_map_usemap_fragment(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-xhtml/map-usemap-fragment-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_ops_svg_font_face_src(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_epub2_fixture(fixtures, "ops-document-svg/font-face-src-valid.svg"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
