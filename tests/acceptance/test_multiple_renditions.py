...
def test_multiple_renditions_basic(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_multiple_renditions_fixture(fixtures, "renditions-basic-valid"), transport="subprocess")
...
def test_multiple_renditions_container_required(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_multiple_renditions_fixture(fixtures, "renditions-mapping-non-xhtml-error"), transport="subprocess")
...
def test_multiple_renditions_spine_required(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_multiple_renditions_fixture(fixtures, "renditions-mapping-no-resourcemap-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_multiple_renditions_nav_element(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_multiple_renditions_fixture(fixtures, "renditions-mapping-untyped-nav-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_multiple_renditions_single_data_nav(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_multiple_renditions_fixture(fixtures, "renditions-mapping-multiple-docs-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_multiple_renditions_region_navigation(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_multiple_renditions_fixture(fixtures, "renditions-mapping-multiple-nav-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_multiple_renditions_region_nav_element(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_multiple_renditions_fixture(fixtures, "renditions-selection-attribute-unknown-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_multiple_renditions_region_nav_fixed_layout(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_multiple_renditions_fixture(fixtures, "renditions-selection-mediaquery-syntax-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_multiple_renditions_region_nav_content_model(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_multiple_renditions_fixture(fixtures, "renditions-selection-attribute-missing-error"), transport="subprocess")
    assert result.returncode == 0
...
def test_multiple_renditions_metadata(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_multiple_renditions_fixture(fixtures, "renditions-metadata-identifier-incomplete-error"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_multiple_renditions_version(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_multiple_renditions_fixture(fixtures, "renditions-mapping-no-version-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
