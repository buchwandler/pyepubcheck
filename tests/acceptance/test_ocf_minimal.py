...
# specmason: unmapped=OCF acceptance slice pending imported requirement mapping
...
def test_missing_mimetype_reports_pkg_006(run_pyepubcheck, fixtures) -> None:
    result = run_pyepubcheck(_ocf_fixture(fixtures, "ocf-mimetype-file-missing-error"), transport="subprocess")
...
def test_invalid_mimetype_reports_pkg_007(run_pyepubcheck, fixtures) -> None:
    result = run_pyepubcheck(_ocf_fixture(fixtures, "ocf-mimetype-file-incorrect-value-error"), transport="subprocess")
    assert result.returncode == 1
