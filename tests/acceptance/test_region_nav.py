...
def test_region_nav_not_xhtml(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_region_nav_fixture(fixtures, "data-nav-not-xhtml-error"), transport="subprocess")
...
def test_region_nav_in_spine(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_region_nav_fixture(fixtures, "data-nav-in-spine-warning"), transport="subprocess")
    assert result.returncode == 0
...
def test_region_nav_unidentified_element(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_region_nav_fixture(fixtures, "data-nav-missing-type-error"), transport="subprocess")
    assert result.returncode == 1
...
def test_region_nav_not_on_nav_element(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_region_nav_fixture(fixtures, "region-based-nav-wrong-element-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_region_nav_not_fixed_layout(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_region_nav_fixture(fixtures, "region-based-nav-not-fxl-error"), transport="subprocess")
...
def test_region_nav_invalid_content_model(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_region_nav_fixture(fixtures, "region-based-nav-content-model-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_region_nav_comics_semantics(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck(_region_nav_fixture(fixtures, "region-based-nav-comics-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
