...
def test_dictionary_dc_type_required(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "dict", _profile_fixture(fixtures, "epub-dictionaries", "epub/dictionary-dc-type-missing-error"), transport="subprocess")
...
def test_dictionary_single_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "dict", _profile_fixture(fixtures, "epub-dictionaries", "epub/dictionary-single-valid"), transport="subprocess")
...
def test_dictionary_search_key_map_required(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "dict", _profile_fixture(fixtures, "epub-dictionaries", "epub/dictionary-search-key-map-missing-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_dictionary_type_monolingual_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "dict", _profile_fixture(fixtures, "epub-dictionaries", "epub/dictionary-type-monolingual-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_dictionary_type_unknown_value(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "dict", _profile_fixture(fixtures, "epub-dictionaries", "epub/dictionary-type-unknown-value-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_dictionary_source_language_required(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "dict", _profile_fixture(fixtures, "epub-dictionaries", "epub/dictionary-source-language-missing-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_dictionary_multiple_publication(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "dict", _profile_fixture(fixtures, "epub-dictionaries", "epub/dictionary-multiple-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_dictionary_glossary_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "dict", _profile_fixture(fixtures, "epub-dictionaries", "epub/dictionary-glossary-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_edupub_minimal_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "edupub", _profile_fixture(fixtures, "epub-edupub", "epub/edupub-minimal-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_edupub_dc_type_required(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "edupub", _profile_fixture(fixtures, "epub-edupub", "epub/edupub-dc-type-missing-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_edupub_accessibility_features_required(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "edupub", _profile_fixture(fixtures, "epub-edupub", "epub/edupub-accessibility-features-missing-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_edupub_basic_publication(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "edupub", _profile_fixture(fixtures, "epub-edupub", "epub/edupub-basic-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_edupub_page_list_missing(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "edupub", _profile_fixture(fixtures, "epub-edupub", "epub/edupub-pagelist-missing-error"), transport="subprocess")
    assert result.returncode == 1
...
def test_edupub_page_list_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "edupub", _profile_fixture(fixtures, "epub-edupub", "epub/edupub-pagelist-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_edupub_content_document_minimal(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "edupub", _profile_fixture(fixtures, "epub-edupub", "edupub-content-document-xhtml/minimal.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_edupub_body_as_section(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "edupub", _profile_fixture(fixtures, "epub-edupub", "edupub-content-document-xhtml/body-section-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_edupub_body_section_without_heading(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "edupub", _profile_fixture(fixtures, "epub-edupub", "edupub-content-document-xhtml/body-section-no-heading-error.xhtml"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_edupub_heading_aria_role(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "edupub", _profile_fixture(fixtures, "epub-edupub", "edupub-content-document-xhtml/heading-aria-role-valid.xhtml"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_indexes_minimal_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "idx", _profile_fixture(fixtures, "epub-indexes", "epub/index-minimal-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_indexes_collection_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "idx", _profile_fixture(fixtures, "epub-indexes", "epub/index-collection-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_indexes_publication_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "idx", _profile_fixture(fixtures, "epub-indexes", "epub/index-publication-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_indexes_publication_without_index(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "idx", _profile_fixture(fixtures, "epub-indexes", "epub/index-publication-no-index-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_indexes_single_file_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "idx", _profile_fixture(fixtures, "epub-indexes", "epub/index-single-file-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_indexes_single_file_without_index(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "idx", _profile_fixture(fixtures, "epub-indexes", "epub/index-single-file-no-index-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_indexes_collection_without_index(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "idx", _profile_fixture(fixtures, "epub-indexes", "epub/index-collection-no-index-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_previews_publication_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "preview", _profile_fixture(fixtures, "epub-previews", "epub/preview-publication-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_previews_dc_type_required(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "preview", _profile_fixture(fixtures, "epub-previews", "epub/preview-pub-dc-type-missing-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_previews_source_publication_required(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "preview", _profile_fixture(fixtures, "epub-previews", "epub/preview-pub-source-missing-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_previews_embedded_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "preview", _profile_fixture(fixtures, "epub-previews", "epub/preview-embedded-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_previews_embedded_manifest_required(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "preview", _profile_fixture(fixtures, "epub-previews", "epub/preview-embedded-no-manifest-error"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_previews_collection_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--profile", "preview", _profile_fixture(fixtures, "epub-previews", "epub/preview-collection-valid"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_accessibility_prefix_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--mode", "opf", _profile_fixture(fixtures, "epub-accessibility", "property-prefix-a11y-valid.opf"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_accessibility_unknown_property(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--mode", "opf", _profile_fixture(fixtures, "epub-accessibility", "property-prefix-a11y-unknown-value-error.opf"), transport="subprocess")
    assert result.returncode == 1
...
def test_accessibility_certifier_credential(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--mode", "opf", _profile_fixture(fixtures, "epub-accessibility", "property-prefix-a11y-certifier-credential-valid.opf"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_accessibility_exemption_property(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--mode", "opf", _profile_fixture(fixtures, "epub-accessibility", "property-prefix-a11y-exemption-valid.opf"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_distributable_object_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--mode", "opf", _profile_fixture(fixtures, "epub-distributable-objects", "package-document/do-collection-metadata-valid.opf"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_distributable_object_identifier_required(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--mode", "opf", _profile_fixture(fixtures, "epub-distributable-objects", "package-document/do-collection-metadata-identifier-missing-error.opf"), transport="subprocess")
    assert result.returncode == 1
    assert result.has_error("RSC-005")
...
def test_scriptable_component_valid(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--mode", "opf", _profile_fixture(fixtures, "epub-scriptable-components", "package-document/sc-prefix-declaration-valid.opf"), transport="subprocess")
    assert result.returncode == 0
    assert result.no_other_errors_or_warnings()
...
def test_scriptable_component_prefix_required(run_pyepubcheck, fixtures) -> None:
    ...
    result = run_pyepubcheck("--mode", "opf", _profile_fixture(fixtures, "epub-scriptable-components", "package-document/sc-prefix-declaration-missing-error.opf"), transport="subprocess")
    assert result.returncode == 1
